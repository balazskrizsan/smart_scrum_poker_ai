...
import com.kbalazsworks.ssp_ai_backend.fakers.domain.jira_module.value_objects.CreateJiraIssueFakeBuilder
...
class IssueEmbeddingService_CreateEmbeddingTest : AbstractTest() {
    ...
    fun validEmbedding_savedToDb() {
        // Arrange
        val tested = CreateJiraIssueFakeBuilder().build()

        val excepted = JiraIssueFakeBuilder().build()
        ...
        // Assert
        val actual = getDSLContext().selectFrom(JIRA_ISSUES).fetchOneInto(JiraIssue::class.java)
