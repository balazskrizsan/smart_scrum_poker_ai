/*
 * This file is generated by jOOQ.
 */
package com.kbalazsworks.ssp_ai_backend.db.tables.records


import com.kbalazsworks.ssp_ai_backend.db.tables.Questions
import com.pgvector.PGvector

import java.time.LocalDateTime

import org.jooq.Record1
import org.jooq.impl.UpdatableRecordImpl


/**
 * This class is generated by jOOQ.
 */
@Suppress("UNCHECKED_CAST")
open class QuestionsRecord() : UpdatableRecordImpl<QuestionsRecord>(Questions.QUESTIONS) {

    open var id: Long?
        set(value): Unit = set(0, value)
        get(): Long? = get(0) as Long?

    open var question: String?
        set(value): Unit = set(1, value)
        get(): String? = get(1) as String?

    open var embedding1536: PGvector?
        set(value): Unit = set(2, value)
        get(): PGvector? = get(2) as PGvector?

    open var embedding3072: PGvector?
        set(value): Unit = set(3, value)
        get(): PGvector? = get(3) as PGvector?

    open var createdAt: LocalDateTime?
        set(value): Unit = set(4, value)
        get(): LocalDateTime? = get(4) as LocalDateTime?

    // -------------------------------------------------------------------------
    // Primary key information
    // -------------------------------------------------------------------------

    override fun key(): Record1<Long?> = super.key() as Record1<Long?>

    /**
     * Create a detached, initialised QuestionsRecord
     */
    constructor(id: Long? = null, question: String? = null, embedding1536: PGvector? = null, embedding3072: PGvector? = null, createdAt: LocalDateTime? = null): this() {
        this.id = id
        this.question = question
        this.embedding1536 = embedding1536
        this.embedding3072 = embedding3072
        this.createdAt = createdAt
        resetChangedOnNotNull()
    }
}
